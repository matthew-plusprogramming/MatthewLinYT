...
async function replaceKeyboard(snippetObject) {
  const deleteLength =
    terminatingChar.length + cue.length + snippetObject.toReplace.length;
  for (let i = 0; i < deleteLength; ++i) {
    await keyboard.pressKey(Key.Backspace);
    await keyboard.releaseKey(Key.Backspace);
  }

  keyboard.type(snippetObject.snippet);
}
    ...
    if (msgCharacter === terminatingChar) {
      ...
      Object.entries(wordDictionary).forEach(([key, value]) => {
        if (userCommand === key) {
          ...
          replaceKeyboard({ toReplace: key, snippet: value });
        }
      });
    }
  }
